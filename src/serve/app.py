...
def preprocess_input_data(input_df: pd.DataFrame) -> pd.DataFrame:
    """Preprocess input data to match the model's expected feature format."""
    try:
...
async def health_check(service: MLModelService = Depends(get_ml_service)):
    ...
    mlflow_connectivity = False
    mlflow_error = None
    try:
        if service.mlflow_client:
            service.mlflow_client.search_experiments(max_results=1)
...
@app.post("/admin/reload", summary="Reload Model", tags=["Admin"])
async def reload_model(service: MLModelService = Depends(get_ml_service)):
    """Admin endpoint to reload the model from MLflow Model Registry."""
    try:
