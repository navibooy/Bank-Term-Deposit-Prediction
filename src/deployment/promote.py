...
def load_thresholds(config_path: str = "config.yaml") -> Dict[str, Any]:
    ...
    """
    defaults = {"roc_auc": 0.85, "f1_score": 0.75, "model_name": "champion"}
    ...
    try:
        ...
        thresholds = deployment_config.get("promotion_thresholds", {})
...
def meets_thresholds(
    ...
    for threshold_name, threshold_value in thresholds.items():
        ...
        if metric_value < threshold_value:
            violations.append(
                f"{threshold_name}({metric_value:.4f}) < {threshold_value}"
            )
...
def ensure_registered_version(
    client: MlflowClient, model_name: str, model_uri: str, max_wait_time: int = 120
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Model registered as version {version_number}, waiting for READY status"
        )
        ...
        while time.time() - start_time < max_wait_time:
            try:
                ...
                logger.info(
                    f"Model version {version_number} status: {status}, waiting..."
                )
                time.sleep(5)
        ...
        # Timeout
        logger.warning(
            f"Timeout waiting for model version {version_number} to be ready"
        )
        return {
            "model_name": model_name,
            "version": version_number,
...
def promote_to_champion(
    ...
    if thresholds is None:
        ...
        thresholds = {
            k: v for k, v in config_thresholds.items() if isinstance(v, (int, float))
        }
    ...
    try:
        ...
        try:
            for tag_key, tag_value in default_tags.items():
                client.set_model_version_tag(
                    model_name, str(version_number), tag_key, tag_value
                )
        except Exception as e:
        ...
        logger.info(
            f"Successfully promoted model {run_id} to production as version {version_number}"
        )
...
def reload_fastapi(
    ...
    if reload_url:
        try:
            ...
            return {"reloaded": True, "method": "http", "error": None}
            ...
            if restart_cmd:
                try:
                    logger.info(
                        f"Attempting container restart: {' '.join(restart_cmd)}"
                    )
                    result = subprocess.run(
                        restart_cmd, capture_output=True, text=True, timeout=timeout
                    )

                    if result.returncode == 0:
                        logger.info("FastAPI restarted successfully via command")
                        return {"reloaded": True, "method": "restart", "error": None}
                    else:
                    ...
                    logger.error(error_msg)
                    return {"reloaded": False, "method": "restart", "error": error_msg}

            # Both methods failed
            return {"reloaded": False, "method": "http", "error": str(e)}
    ...
    return {"reloaded": False, "method": "none", "error": "No reload method provided"}
